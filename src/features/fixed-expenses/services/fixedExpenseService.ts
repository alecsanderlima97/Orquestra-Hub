...
export async function createFixedExpense(tenantId: string, expense: Omit<FixedExpense, "id">) { if (!firebaseReady || !db) return null; return addDoc(collection(db, tenantCollectionPath(tenantId, collectionName)), { ...expense, createdAt: serverTimestamp() }); }
