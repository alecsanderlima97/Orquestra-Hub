  ...
  const printReport = () => {
    ...
    reportWindow.document.write(`<!doctype html><html lang="pt-BR"><head><meta charset="utf-8" /><title>Relatório financeiro</title><style>
      * { box-sizing: border-box; } body { margin: 0; padding: 28px; color: #0f172a; font-family: Arial, sans-serif; background: #fff; }
      ...
      <h2>Pagamentos do período</h2><table><thead><tr><th>Fornecedor</th><th>Categoria</th><th>Loja</th><th>Parcela</th><th>Vencimento</th><th>Valor</th><th>Status</th></tr></thead><tbody>${accountRows}</tbody></table>
      <h2>Compras/notas lançadas no período</h2><table><thead><tr><th>Nota</th><th>Data</th><th>Fornecedor</th><th>Loja</th><th>Descrição</th><th>Total</th></tr></thead><tbody>${purchaseRows}</tbody></table>
    </body></html>`);
...
function ReportTable({ accounts }: { accounts: AccountPayable[] }) { return <div className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm"><div className="border-b border-slate-200 px-5 py-4"><h3 className="font-semibold">Pagamentos do período</h3></div><div className="max-h-[460px] overflow-auto"><table className="w-full min-w-[860px] text-left text-sm"><thead className="sticky top-0 z-10 bg-slate-50 text-slate-600"><tr><th className="px-5 py-3">Fornecedor</th><th className="px-5 py-3">Categoria</th><th className="px-5 py-3">Loja</th><th className="px-5 py-3">Parcela</th><th className="px-5 py-3">Vencimento</th><th className="px-5 py-3">Valor</th><th className="px-5 py-3">Status</th></tr></thead><tbody className="divide-y divide-slate-100">{accounts.length ? accounts.map((item) => <tr key={item.id}><td className="px-5 py-3 font-medium">{item.supplier}</td><td className="px-5 py-3">{item.categoryName || "Sem categoria"}</td><td className="px-5 py-3">{item.store}</td><td className="px-5 py-3">{item.installment}</td><td className="px-5 py-3">{item.dueDate}</td><td className="px-5 py-3 font-medium">{item.amount}</td><td className="px-5 py-3">{item.status}</td></tr>) : <tr><td className="px-5 py-8 text-center text-slate-500" colSpan={7}>Nenhum pagamento encontrado.</td></tr>}</tbody></table></div></div>; }
function PurchasesReportTable({ purchases }: { purchases: Purchase[] }) { return <div className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm"><div className="border-b border-slate-200 px-5 py-4"><h3 className="font-semibold">Compras/notas lançadas no período</h3></div><div className="max-h-[460px] overflow-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead className="sticky top-0 z-10 bg-slate-50 text-slate-600"><tr><th className="px-5 py-3">Nota</th><th className="px-5 py-3">Data</th><th className="px-5 py-3">Fornecedor</th><th className="px-5 py-3">Loja</th><th className="px-5 py-3">Descrição</th><th className="px-5 py-3">Total</th></tr></thead><tbody className="divide-y divide-slate-100">{purchases.length ? purchases.map((item) => <tr key={item.id}><td className="px-5 py-3 font-medium">{item.invoiceNumber}</td><td className="px-5 py-3">{item.issueDate}</td><td className="px-5 py-3">{item.supplier}</td><td className="px-5 py-3">{item.store}</td><td className="px-5 py-3">{item.description || "Não informado"}</td><td className="px-5 py-3 font-medium">{item.total}</td></tr>) : <tr><td className="px-5 py-8 text-center text-slate-500" colSpan={6}>Nenhuma compra/nota encontrada.</td></tr>}</tbody></table></div></div>; }
