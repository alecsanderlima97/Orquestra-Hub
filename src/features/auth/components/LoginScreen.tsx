...
export function LoginScreen({ onLogin }: { onLogin: (user: AppUser) => void }) {
  ...
  async function handleReset() { if (!email) { setMessage("Informe seu e-mail para recuperar a senha."); return; } try { await resetPassword(email); setMessage("Enviamos o link de recuperação para seu e-mail."); } catch { setMessage("Não foi possível enviar o link de recuperação."); } }
