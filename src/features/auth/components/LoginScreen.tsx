...
export function LoginScreen({ onLogin }: { onLogin: (user: AppUser) => void }) {
  ...
  async function handleGoogle() { setMessage(""); if (mode === "register" && !acceptedTerms) { setMessage("Aceite os Termos de Uso e a Política de Privacidade."); return; } setLoading(true); try { const user = await loginWithGoogle(); if (user) await finishLogin(user); } catch { setMessage("Não foi possível acessar com o Google. Verifique se o provedor está habilitado."); } finally { setLoading(false); } }
