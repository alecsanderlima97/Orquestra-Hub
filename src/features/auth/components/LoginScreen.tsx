...
function googleLoginError(error: unknown) {
  ...
  if (code.includes("popup-closed-by-user") || code.includes("cancelled-popup-request")) return "A janela do Google foi fechada antes da conclusão.";
  if (code.includes("unauthorized-domain")) return "Este domínio ainda não foi autorizado no Firebase Authentication.";
  if (code.includes("operation-not-allowed")) return "O login Google ainda não foi habilitado no Firebase Authentication.";
  if (code.includes("network-request-failed")) return "Falha de conexão com o Google. Verifique sua internet.";
  return "Não foi possível concluir o acesso pelo Google. Tente novamente ou entre com e-mail e senha.";
}
...
export function LoginScreen({ onLogin }: { onLogin: (user: AppUser) => void }) {
  ...
  async function handleSubmit() { setMessage(""); setLoading(true); try { if (!firebaseReady) { setMessage("Firebase não configurado. Configure o .env.local para salvar dados reais ou use somente o modo demo."); return; } const user = mode === "register" ? await registerWithEmail(name, companyName, email, password, inviteCode, acceptedTerms) : await loginWithEmail(email, password); if (user) await finishLogin(user); } catch (error) { setMessage(mode === "login" ? "E-mail ou senha inválidos." : registrationError(error)); } finally { setLoading(false); } }
  async function handleGoogle() { setMessage(""); if (mode === "register" && !acceptedTerms) { setMessage("Aceite os Termos de Uso e a Política de Privacidade."); return; } setLoading(true); try { const user = await loginWithGoogle(); if (user) await finishLogin(user); } catch (error) { setMessage(googleLoginError(error)); } finally { setLoading(false); } }
  async function handleReset() { if (!email) { setMessage("Informe seu e-mail para recuperar a senha."); return; } try { await resetPassword(email); setMessage("Solicitação enviada pela Orquestra Hub. Verifique também Spam e Promoções."); } catch (error) { setMessage(error instanceof Error ? error.message : "Não foi possível enviar o link de recuperação."); } }
