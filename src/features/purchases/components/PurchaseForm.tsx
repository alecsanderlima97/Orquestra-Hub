...
export function PurchaseForm({ error, form, onChange, onSubmit, storeOptions, supplierOptions }: { error?: string; form: PurchaseFormState; onChange: (form: PurchaseFormState) => void; onSubmit: () => void; storeOptions: string[]; supplierOptions: string[] }) {
  return <form className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      ...
      <button className="mt-7 h-11 rounded-md bg-slate-950 px-4 text-sm font-semibold text-white hover:bg-slate-800" onClick={onSubmit} title="Lançar a nota e criar automaticamente as parcelas." type="button">Gerar parcelas</button>
    </div>
    <div className="mt-4"><FormAlert message={error} /></div>
  </form>;
}
