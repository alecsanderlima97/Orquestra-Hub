  ...
  return <div className="mt-4 overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
    ...
    <table className="w-full min-w-[980px] text-left text-sm">
      ...
      <tbody className="divide-y divide-slate-100">{purchases.length ? purchases.map((purchase) => <tr key={purchase.id}>
        <td className="px-5 py-4 font-medium">{purchase.invoiceNumber}</td><td className="px-5 py-4">{purchase.supplier}</td><td className="max-w-[260px] px-5 py-4 text-slate-700">{purchase.description || "Não informado"}</td><td className="px-5 py-4">{purchase.store}</td><td className="px-5 py-4">{purchase.issueDate}</td><td className="px-5 py-4 font-medium">{purchase.total}</td><td className="px-5 py-4">{purchase.installments}</td><td className="px-5 py-4"><button aria-label="Editar nota" className="rounded-md border border-slate-200 p-2 hover:bg-amber-50 hover:text-amber-800" onClick={() => onEdit?.(purchase)} title="Editar os dados desta nota" type="button"><Pencil size={17} /></button></td>
      </tr>) : <tr><td className="px-5 py-8 text-center text-slate-500" colSpan={8}>Nenhuma nota lançada ainda.</td></tr>}</tbody>
    </table>
  </div>;
}
