...
export async function listTenantUsers(tenantId: string, companyName = "Empresa"): Promise<AppUser[]> {
  ...
  return snap.docs.map((item) => ({ companyName, tenantId, id: item.id, email: String(item.data().email || ""), name: String(item.data().name || item.data().email || "Usuário"), role: (item.data().role || "Consulta") as AppUser["role"] }));
}
