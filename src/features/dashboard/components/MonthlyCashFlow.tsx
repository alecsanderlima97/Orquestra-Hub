    ...
    <div className="theme-surface mt-6 rounded-lg border border-slate-200 bg-white shadow-sm">
      ...
      <div className="grid gap-3 p-5 sm:grid-cols-2 xl:grid-cols-3">
        {rows.map(([month, values]) => (
          <article className="rounded-md border border-slate-200 p-4 dark:border-slate-600" key={month}>
            <strong>{month}</strong>
            <div className="mt-3 flex justify-between text-sm">
              <span className="text-emerald-700 dark:text-emerald-300">Pago: {money.format(values.paid)}</span>
              <span className="text-amber-800 dark:text-amber-300">Aberto: {money.format(values.open)}</span>
            </div>
          </article>
        ))}
      </div>
